...
//       }
//     } else {
//       returnText += (isdir ? "Dir: " : "File: ") + String(foundfile.name()) +  (isdir ? "/" :"") + " Size: " + server_ui_size(foundfile.size()) + "\n";
//     }
...
static String server_directory_row(File root, bool ishtml)
{
  String returnText = "";
  File foundfile = root.openNextFile();
  while (foundfile)
  {
    ...
    if (!isdir)
    {
      if (ishtml)
      {
        returnText += "<tr align='left'><td>" + String(foundfile.path()) + "</td><td>" + server_ui_size(foundfile.size()) + "</td>";
        returnText += "<td><button class='directory_buttons' onclick=\"directory_button_handler(\'" + String(foundfile.path()) + "\', \'download\')\">Download</button>";
        returnText += "<td><button class='directory_buttons' onclick=\"directory_button_handler(\'" + String(foundfile.path()) + "\', \'delete\')\">Delete</button></tr>";
      }
      else
      {
    ...
    {
      returnText += server_directory_row(foundfile, ishtml);
    }
    foundfile = root.openNextFile();
  }
  root.close();
...
static String server_directory(bool ishtml)
{
  ...
  File root = SPIFFS.open("/");
  returnText += server_directory_row(root, ishtml);
  if (ishtml)
  {
    returnText += "</table>";
  }
...
void initFileAsyncEndpoints(AsyncWebServer *server)
{
  ...
  server->on("/directory", HTTP_GET, [](AsyncWebServerRequest *request)
    ...
    String logmessage = "Client:" + request->client()->remoteIP().toString() + " " + request->url();
    Serial.println(logmessage);
    bool is_html = request->hasParam("html") && request->getParam("html")->value().equalsIgnoreCase("true");
    String ss = server_directory(is_html);
    stats("Directory");
    request->send(200, "text/plain", ss); });
